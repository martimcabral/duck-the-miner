...
func update_current_resources_amount():
	...
	if raw_resources.to_string() != "":
		...
		amount_Ice = raw_resources.get_value("inventory", "Ice", 0)
	...
	if crafted_resources.to_string() != "":
		...
		amount_Hydrogen = crafted_resources.get_value("inventory", "Hydrogen", 0)
